...
class BuildlistController extends Controller{
    ...
    **/
    ...
    public function index($name =NULL,$searchkey=NULL) {//die('52');
            ...
            if($filtertypee !='')
            {
                ...
                // print_r($serchinfo);die('115');
                    if(!empty($serchinfo)) {
                        ...
                        if($save_countryfilter == "") {$save_countryfilter = '{"country":["United States"]}';}
       ...
       // $country = DB::table('internationals')->select('name')->whereNull('deleted_at')->get();
        ...
        $userinfo = DB::table('users')->select('*')->where('id',$this->user_id)->first();
        ...
        // echo $readymadelisids;die('605');
         // print_r($contacts);die('664');
            ...
            'range_of_contact'=>isset($range_of_contact) ? $range_of_contact : '',/** RANGE OF STELCTED IN SLIDER **/
            'range_of_contact_count'=>isset($totlacontact) ? $totlacontact : '',/**RANGE OF TOTAL FILTER DATA**/
    ...
    */
    public function downloadleads(Request $request) {
        ...
        foreach ($result as $key => $value) {    
            ...
            }else {die('Job Title did not set corectly');}
        }
    ...
    */
    ...
    public function downloadcontacts(Request $request) {
                ...
                foreach ($job_function as $key => $value) { 
                    ...
                    for($i = 0 ;$i < count($job_or_keywords) ; $i++){                    
                        ...
                        for($j = 0 ;$j < count($job_and_keywords); $j++) {
                            ...
                            if($j ==0){$where .= '(';}
                            $where .= $st;
                            if($j < count($job_and_keywords) - 1) {$where .= ' and ';}
                            else if($i < count($job_or_keywords) - 1){$where .= ') or ';}
                            else if($key < $total) {$where .= ')) or';}
                            else if($key == $total  && !empty($fields) ){$where .='))) and ';}
                            else if($total==$key ) {$where .= ')))';}
                        }
                    }
                }
            }
        ...
        if(!isset($job_format)){ $job_format = 0;}
        $userid = Session::get('user_id');
        $user_credit = DB::table('users')->whereNull('deleted_at')->where('id',$userid)->first()->credit;
    ...
    public function viewcontact(Request $request) {
        ...
        return response()->json(array('contact'=>$contact,'nocredit'=>false,'credit'=>$credit));die();
    }
    public function checkcontact(Request $request) {
        ...
        return response()->json(array('saved'=>true,'contact'=>$contact,'nocredit'=>false,'credit'=>$credit));die();
    }
    public function savecontact(Request $request) {
        ...
        return response()->json(array('saved'=>true));die();
    }
    /*
    ...
    public function savefilter(Request $request) {
        ...
        $savedfilters = DB::table('savefilters')->select('filtername')->whereNull('deleted_at')->where('userid',$userid)->get('filtername');
        return response()->json(array('savedfilters'=>$savedfilters));die();
    ...
    public function openfilter(Request $request) {
        ...
        return response()->json(array('filters'=>$filter));die();
    }
    public function myleadspage(Request $request) {
        ...
        return response()->json(array('result'=>$myleads));die();
    }
    /**
    ...
    **/
    public function filter(Request $request){
        ...
        if(!isset($job_format)){ $job_format = 0;}
        return response()->json(array('job_format'=>$job_format,'result'=>$final_result, 'count' => $counts, 'counts'=>$counten, 'price' => $price,'prices'=>$priceen,'save_result'=>$concatvalue,'lastid'=>$lastid));die();
    }
    ...
    public function ranges_of_contact(Request $request){
        if($request->ajax()) {
            ...
            if( isset($url_details[3]) && !empty($url_details[3])  ){
                ...
                return response()->json(array( 'count' => $totlacontact, 'price' => $price,'save_result'=>$concatvalue,'prices'=>$priceen,'counts'=>$counten));die();
        }else {
            return response()->json('Anauthorized aceess');die();
        }
    ...
    **/ 
    function getPrice($counts=NULL){
        ...
        if(isset($counts) && $counts>0){//die('1553');
            ...
            switch ($counts) {
                case $counts >0       && $counts <=549      :$price = 99 ;break;
                case $counts >549     &&  $counts<=999      :$price =0.19*$counts ;break;
                ...
                case $counts >=500001   :$price =0.05*$counts ;break;
            }
                ...
                if(!empty($data)){
                   ...
                   // print_r($details);die('934');
                    ...
                    if( !empty($details) ){
                        ...
                        return response()->json(array( 'details' => $details));die();
                    }
                }
    ...
    **/ 
    ...
    function mail_check($name =NULL,$type=null){//die('dd');
