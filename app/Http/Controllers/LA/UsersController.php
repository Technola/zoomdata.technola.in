...
class UsersController extends Controller
{
	...
	public function change_plan(Request $request) {
		...
		if($subscription && empty($api_error) && $flag != 'cancel') {
			...
			if($subsData['status'] == 'active'){ 
				...
				if($subscription_id && !empty($id)){
					...
					if($flag == 'create') {
						...
						Mail::send(['text'=>'mail'], $data, function($message) use ($email,$type,$billing){
							$message->to($email, 'Payment')->subject
								('You created subscription.Now you have ' .$type.'/'.$billing.'  subscription.');
					...
					if($flag == 'upgrade') {
						...
						Mail::send(['text'=>'mail'], $data, function($message) use ($email,$type,$billing){
							$message->to($email, 'Payment')->subject
								('You upgraded subscription.Now you have ' .$type.'/'.$billing.'  subscription.');
							$message->from('debashis.matainja@gmail.com','beta globleads  Payment Information');
						});
						Mail::send(['text'=>'mail'], $data, function($message) use ($email,$type,$billing){
							$message->to('debashis.matainja@gmail.com', 'Payment')->subject
 					...
 					if($flag == 'downgrade') {
					   ...
					   Mail::send(['text'=>'mail'], $data, function($message) use ($email,$type,$billing){
						   $message->to($email, 'Payment')->subject
							   ('You downgraded subscription.Now you have ' .$type.'/'.$billing.'  subscription.');
						   $message->from('debashis.matainja@gmail.com','beta globleads  Payment Information');
					   });
					   Mail::send(['text'=>'mail'], $data, function($message) use ($email,$type,$billing){
						   $message->to('debashis.matainja@gmail.com', 'Payment')->subject
			...
			Mail::send(['text'=>'mail'], $data, function($message) use ($email){
				$message->to($email, 'Payment')->subject
					('You cancelled subscription.Now you have free subscription.');
				$message->from('debashis.matainja@gmail.com','beta globleads  Payment Information');
			});
			Mail::send(['text'=>'mail'], $data, function($message) use ($email){
				$message->to('debashis.matainja@gmail.com', 'Payment')->subject
	...
	public function send_message(Request $request) {
		...
		Mail::send(['text'=>'mail'], $data, function($message) use ($email,$msg){
			$message->to($email, 'FromAdmin')->subject
				($msg);
