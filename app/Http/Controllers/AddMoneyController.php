...
class AddMoneyController extends HomeController
{
    ...
    public function cancelSubscription() {
        ...
        if($subscription != '' && empty($api_error)) {
            ...
            Mail::send(['text'=>'mail'], $data, function($message) use ($email){
                $message->to($email, 'Payment')->subject
                    ('You have cancelled subscription.');
                $message->from('debashis.matainja@gmail.com','beta globleads  Payment Information');
            });
            Mail::send(['text'=>'mail'], $data, function($message) use ($email){
                $message->to('debashis.matainja@gmail.com', 'Payment')->subject
                    ('You have cancelled subscription.');
                $message->from('debashis.matainja@gmail.com', 'beta globleads  Payment Information');
            });
        ...
        return Redirect::route('tool/upgrade-plan');
    }
    public function cardpay(Request $request) {
        $api_error = '';
        $userid = Session::get('user_id');
        if(empty($userid)){
            return Redirect::route('tool/advanced-search');
        }
                    ...
                    if($subsData['status'] == 'active'){ 
                        ...
                        if($subscription_id && !empty($userid)){
                            if($flag == 'create' || $flag=='upgrade') {
                                ...
                                if($flag == 'upgrade') {
                                    DB::table('users')->where('id',$userid)->update(array('last_upgrade' => $date, 'subscription_id' => $subscription_id, 'plan' => $planInfoID));
                                } else {
                                    $credit = 0;
                                    DB::table('users')->where('id',$userid)->update(array('credit' => 0, 'last_upgrade' => $date, 'subscription_id' => $subscription_id, 'plan' => $planInfoID));
                                ...
                                Mail::send(['text'=>'mail'], $data, function($message) use ($email,$type){
                                    $message->to($email, 'Payment')->subject
                                        ('You upgraded subscription.Now you have ' .$type.'  subscription.');
                                    $message->from('debashis.matainja@gmail.com','beta globleads  Payment Information');
                                });
                                Mail::send(['text'=>'mail'], $data, function($message) use ($email,$type){
                                    $message->to('debashis.matainja@gmail.com', 'Payment')->subject
                               ...
                               DB::table('users')->where('id',$userid)->update(array('last_downgrade' => $date,'down_plan' => $planID, 'down_date' => date("Y-m-d H:i:s", $subscription->cancel_at)));
                               $html = array('msg'=>'You downgraded subscription.Now you have ' .$type.'  subscription.');  
                               $data = array('name'=>"beta globleads  Payment Information",'email'=>$email,'type'=>$type);  
                                Mail::send(['text'=>'mail'], $data, function($message) use ($email,$type){
                                    $message->to($email, 'Payment')->subject
                                        ('You downgraded subscription to ' .$type.'  subscription.');
                                    $message->from('debashis.matainja@gmail.com','beta globleads  Payment Information');
                                });
                                Mail::send(['text'=>'mail'], $data, function($message) use ($email,$type){
                                    $message->to('debashis.matainja@gmail.com', 'Payment')->subject
    ...
    public function addpayment($type,$billing){
        ...
        if(!empty($userid)){
            ...
            $item_1 = new Item();
            $item_1->setName('Search-'.$userid.'-'.$price) /** item name **/
                ->setCurrency('USD')
                ->setQuantity(1)
                ->setPrice($price);
            ...
            try {
                ...
                if (\Config::get('app.debug')) {
                    ...
                    return Redirect::route('checkout/'.$backurl);
        ...
        if(!empty($userid)){
           ...
           //  print_r($payment);die('288');
            ...
            if ($result->getState() == 'approved') { 
                ...
                $randomStringfirst = '';
                $randomStringlast='';
                $html = array('paypaltransactionid'=>$TransactionIdpaypal,
                                'status'=>'success',
                                'amoutn'=> $paidPaypalPrice);  
                Mail::send('maildown', $html, function($message) {
                    $message->to($_POST['email'], 'Payment')->subject
                        ('Payment success');
                    $message->from('debashis.matainja@gmail.com','beta globleads  Payment Information');
                });
                Mail::send('maildown', $html, function($message) {
                ...
                $dataPayemnt = array('user_id'=>$userid,
